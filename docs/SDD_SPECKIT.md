...
> Aplicada aquí al **módulo Gestión Profesoral**. El método es el mismo en
...
este curso usted lo vive: la [GUIA_IA.md de la versión](spec_kit/versiones/v1_programa/GUIA_IA1.md) construye la versión
...
- **La versión en curso:**
  [spec_kit/versiones/v1_programa/](spec_kit/versiones/v1_programa/2_spec.md)
...
`PATCH /api/programa/{codigo}` con body de la petición ProgramaActualizar:
...
| Tabla | PK | Semilla |
|---|---|---|
| programa | id | 191 filas del Excel del módulo |
...
```bash
curl http://localhost:8029/api/programa        # total: 8
curl -i http://localhost:8029/api/programa/PR999   # → 404
```
...
- [ ] models/Programa.py (la entidad: 4 propiedades tipadas)
- [ ] models/ProgramaCrear.py (todo obligatorio, con [Required])
...
```markdown
### RF6 — Retirar el programa
DELETE /api/programa/{id} retira el programa del uso.
...
|---|---|---|---|---|---|
| RF1 — listar | C1, C2 | §2 `GET /api/programa` | Fase 5 · controlador | §2 comando 2 | 1 y 2 |
...
clarify NO crea un archivo: escribe dentro de 2_spec.md
    ## Clarificaciones
    C5 — DELETE, ¿físico o lógico? -> LÓGICO: la tabla programa gana
...
4. **Es evaluable y comparable.** Los `.md` se leen y se califican, y los
   cursos comparten la misma estructura sobre la misma `gestion_local` con
   stacks distintos.
...
4. En este repositorio: el [spec kit completo](spec_kit/1_constitution.md)
   y la [GUIA_IA.md de la versión](spec_kit/versiones/v1_programa/GUIA_IA1.md) que lo pone a prueba.
