...
`RepositorioProgramaPostgreSQL` encapsula el engine de conexión y el SQL: nadie
...
es **composición sobre herencia** — y este proyecto la sigue: `ServicioPrograma`
...
`IRepositorioPrograma` puede ocupar el lugar de otra — el PostgreSQL real, el
...
classDiagram
    ...
    RepositorioProgramaPostgreSQL ..|> IRepositorioPrograma : POLIMORFISMO
    ...
    note for RepositorioProgramaPostgreSQL "ENCAPSULAMIENTO: la conexión y el SQL no salen de aquí"
```
...
modelo Pydantic: `class Programa(BaseModel)` — Programa ES-UN BaseModel y
...
classDiagram
    ...
    ServicioMal --|> RepositorioProgramaPostgreSQL : hereda del CONCRETO — quedó casado con PostgreSQL
...
sequenceDiagram
    ...
    participant R as IRepositorioPrograma (Protocol)
...
Programa(codigo="PR009", nombre="Webcam", stock=5, valorunitario=120000)  # ✔ objeto válido
Programa(codigo="PR009", nombre="Webcam", stock=-5, valorunitario=1)      # ✖ ValidationError
```

Mírelo con lentes de paradigma — Pydantic es dos paradigmas cooperando:

- Es **P.O.O.**: `Programa` es una clase; cada petición válida se convierte en
...
1. **Frontera de entrada:** FastAPI recibe el body JSON, intenta construir el
   ...
   a la BD.
2. **Un modelo por semántica HTTP:** `Programa` (POST: todo obligatorio),
   `ProgramaReemplazo` (PUT: reemplazo completo) y `ProgramaActualizar`
...
```
Programa (Pydantic)          ← objeto de DATOS con validación (abstracción del dominio)
ServicioPrograma             ← objeto de NEGOCIO; compone un IRepositorioPrograma
IRepositorioPrograma         ← contrato (Protocol): abstracción pura
RepositorioProgramaPostgreSQL ← implementación concreta (encapsula SQL y conexión)
...
(`RepositorioProgramaMysqlMariaDB`) entrará por la misma puerta.
