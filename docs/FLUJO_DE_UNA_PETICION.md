...
```python
@router.get("/programa")            # ← ESTO es "el GET de la colección"
async def listar_programas(limite: int = 1000):
    ...

@router.post("/programa")           # ← y ESTO es "el POST"
async def crear_programa(programa: Programa):
    ...
```

El decorador registra la pareja **(verbo, ruta) → función** en una tabla de
rutas. Cuando uvicorn recibe `POST /api/programa`, FastAPI busca esa pareja
en la tabla y llama `crear_programa(...)`. Su función es "el POST"
...
> ruta == "/api/programa": ...`. FastAPI la esconde — el decorador es
...
|---|---|---|
| Las 6 rutas de programa | `controllers/programa_controller.py` | `router = APIRouter(prefix="/api", tags=["Programa"])` + un decorador por endpoint |
...
|---|---|---|
| **Parámetro de ruta** | Aparece en la ruta entre llaves Y como argumento | `@router.get("/programa/{codigo}")` + `def obtener_programa(codigo: str)` |
| **Query string** | Argumento con valor por defecto que NO está en la ruta | `def listar_programas(limite: int = 1000)` captura `?limite=3` |
| **El body JSON** | Argumento tipado con un modelo Pydantic | `def crear_programa(programa: Programa)` |
...
función un objeto `Programa` ya construido. Si el body no cumple, responde
...
2. FastAPI           busca (POST, /api/programa) en la tabla de rutas
3. FastAPI/Pydantic  valida el body contra el modelo Programa
      ├─ ¿inválido?  → responde 422 con la lista de errores y AQUÍ TERMINA
      └─ ¿válido?    → llama crear_programa(programa)
4. programa_controller.crear_programa
      try: servicio.crear(programa.model_dump()) …
5. ServicioPrograma.crear      (reglas de negocio; no conoce HTTP)
6. RepositorioProgramaPostgreSQL.crear
      INSERT INTO programa (...) VALUES (:codigo, :nombre, ...)  ← parametrizado
...
```mermaid
flowchart TD
    A["1. uvicorn recibe<br/>el texto HTTP"] --> B["2. FastAPI encuentra<br/>POST /api/programa<br/>en la tabla de rutas"]
...
```
1. FastAPI busca (GET, /api/programa/{codigo}) y extrae codigo="PR001"
2. programa_controller.obtener_programa("PR001")
3. ServicioPrograma.obtener   pide al repositorio; si llega None → lanza
                              LookupError (el except la vuelve 404)
4. RepositorioProgramaPostgreSQL   SELECT ... WHERE codigo = :codigo
...
Invoke-RestMethod "http://localhost:8029/api/programa"
Invoke-RestMethod "http://localhost:8029/api/programa/PR001"

# POST — crear
Invoke-RestMethod -Method Post -Uri "http://localhost:8029/api/programa" -ContentType "application/json" -Body '{"codigo":"PR009","nombre":"Webcam","stock":10,"valorunitario":350000}'

# PUT con body incompleto → error 422 (PUT exige TODO)
Invoke-RestMethod -Method Put -Uri "http://localhost:8029/api/programa/PR009" -ContentType "application/json" -Body '{"stock":25}'

# PATCH con el MISMO body → 200 (PATCH es parcial)
Invoke-RestMethod -Method Patch -Uri "http://localhost:8029/api/programa/PR009" -ContentType "application/json" -Body '{"stock":25}'

# DELETE — limpiar
Invoke-RestMethod -Method Delete -Uri "http://localhost:8029/api/programa/PR009"
```

> Si está probando **SU reconstrucción** (la de la [GUIA_IA1](spec_kit/versiones/v1_programa/GUIA_IA1.md),
> que corre con puertos +100): cambie `8029` por `8105`.
...
con clics en **http://localhost:8029/docs** (Swagger) o con la colección de
[postman/gestion_v1.postman_collection.json](../postman/gestion_v1.postman_collection.json).
