...
tiene 2: `./api_gestion`, `./front_flask`.
...
| `EXPOSE 8029` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
- En la v1: `bd_v1` es un contenedor creado desde la imagen `postgres:16-alpine`,
  con el puerto interno 5432 **publicado** en el 15441 de su PC (`-p 15441:5432`).
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTFLASK["SERVIDOR WEB (front)<br/>contenedor front-flask<br/>hostname: front-flask · escucha en 8027"]
            APIGESTION["SERVIDOR DE APLICACIONES<br/>contenedor api-gestion<br/>hostname: api-gestion · escucha en 8029"]
            POSTGRES[("SERVIDOR DE BASE DE DATOS<br/>PostgreSQL · contenedor postgres<br/>hostname: postgres · escucha en 5432")]
        end
    end
    NAV -->|"localhost:8029"| APIGESTION
    NAV -->|"localhost:8027"| FRONTFLASK
    APIGESTION -->|"postgres:5432 (DNS de Docker)"| POSTGRES
    FRONTFLASK -->|"http://api-gestion:8029<br/>al controlador, por el NOMBRE"| APIGESTION
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15441"| POSTGRES
```
...
services:                          # el mapa de TODOS los contenedores del sistema
  ...
  postgres:                        # ← este nombre es también su HOSTNAME interno
    ...
    ports:
      - "15441:5432"               # "puerto en su PC : puerto interno del contenedor"
    healthcheck:                   # cómo saber si la BD ya RESPONDE (no solo "existe")
      test: ["CMD-SHELL", "pg_isready -U paradigmas -d gestion_local"]
      interval: 5s
      timeout: 5s
      retries: 10
  ...
  api-registros:
    ...
    volumes:
      - ./api_gestion:/app        # el código montado: guardar un .py recarga la API sola
    ...
    ports:
      - "8029:8029"                # http://localhost:8029/docs
    environment:
      ...
      DB_POSTGRES: postgresql+asyncpg://paradigmas:paradigmas123@postgres:5432/gestion_local
    depends_on:
      postgres:
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8029`, `localhost:15441`); entre contenedores, nombres de
...
docker network create proyecto_paradigmas_gestion_profesoral1_default

# 2. postgres
docker run -d --name paradigmas-gestion-postgres --network proyecto_paradigmas_gestion_profesoral1_default --restart unless-stopped `
  -e "POSTGRES_USER=gestion" `
  -e "POSTGRES_PASSWORD=Gestion123!" `
  -e "POSTGRES_DB=gestion_local" `
  -v pgdata:/var/lib/postgresql/data `
  -v "${PWD}/db/init.sql:/docker-entrypoint-initdb.d/init.sql:ro" `
  -p 15441:5432 postgres:16
...
docker run -d --name paradigmas-gestion-api --network proyecto_paradigmas_gestion_profesoral1_default --restart unless-stopped `
  -e "DB_POSTGRES=postgresql+asyncpg://gestion:Gestion123!@postgres:5432/gestion_local" `
  -v "${PWD}/api_gestion:/app" `
  -p 8029:8029 api-gestion uvicorn main:app --host 0.0.0.0 --port 8029 --reload

# 5. Construir la imagen de front-flask y encenderla
docker build -t front-flask ./front_flask
docker run -d --name gestion-front --network proyecto_paradigmas_gestion_profesoral1_default --restart unless-stopped `
  -e "URL_API_GESTION=http://api-gestion:8029" `
  -v "${PWD}/front_flask:/app" `
  -p 8027:8027 front-flask
...
|---|---|---|
| **Su navegador** | `http://localhost:8029` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-gestion:8029` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8029:8029`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8029`, el front
...
```powershell
docker network inspect proyecto_paradigmas_gestion_profesoral1_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_paradigmas_gestion_profesoral1` produce
la red `proyecto_paradigmas_gestion_profesoral1_default`. Tres formas de confirmarlo:
...
network proyecto_paradigmas_gestion_profesoral1_default has active endpoints (name:"gestion-front" …)
```
