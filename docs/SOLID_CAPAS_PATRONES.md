...
│ controllers/programa_controller.py                  │
...
│ repositorios/repositorio_programa_postgresql.py     │
│ El SQL: traduce filas ↔ objetos. NO conoce HTTP.    │
│ NO decide negocio.                                  │
└────────────────┬────────────────────────────────────┘
                 │  ④ SELECT … FROM programa WHERE codigo = 'PR001'
                 ▼
          ┌───────────────┐
          │ BASE DE DATOS │  PostgreSQL — gestion_local
          └───────┬───────┘
                  │
...
|---|---|---|---|
| **Controller** | HTTP: rutas, códigos de estado, JSON | SQL y reglas de negocio | `controllers/programa_controller.py` |
| **Servicio** | Las reglas del negocio (¿existe? ¿se puede?) | Saber de HTTP o del motor de BD | `servicios/servicio_programa.py` |
| **Repositorio** | El SQL y el mapeo fila ↔ objeto | Saber de HTTP o decidir negocio | `repositorios/repositorio_programa_postgresql.py` |
...
Pregunta legítima: la carpeta de modelos (`models/programa.py (Pydantic)`) existe en el
...
- El **modelo es LO QUE VIAJA entre estaciones**: el repositorio arma un
  `Programa` desde la fila, el servicio lo razona, el controller lo
...
@router.get("/api/programa/{codigo}")
...
(`RepositorioProgramaMysqlMariaDB`) y una línea en la fábrica — controllers y
...
class RepositorioProgramaMariaDB:
    """Los mismos 5 métodos que promete IRepositorioPrograma."""

# ...y el ensamblador (ÚNICO archivo tocado) elegirá el motor:
repositorio = (RepositorioProgramaMariaDB(cadena)
               if motor == "mariadb"
               else RepositorioProgramaPostgreSQL(cadena))
```

### L — Sustitución de Liskov (*Liskov Substitution*)
> Donde sirve el tipo base, debe servir CUALQUIER implementación, sin sorpresas.

**Su momento son la v3 y la v4:** los tres repositorios de programa
...
servicio = ServicioPrograma(RepositorioFalso())   # ← el servicio NI SE ENTERA
```
...
class IRepositorioPrograma(Protocol):
...
**En la v1:** `ServicioPrograma` recibe **la interfaz** por constructor; solo
...
class ServicioPrograma:
    def __init__(self, repositorio: IRepositorioPrograma):   # ← interfaz, no clase
...
def crear_servicio_programa() -> IServicioPrograma:
    repositorio = RepositorioProgramaPostgreSQL(os.environ["DB_POSTGRES"])
...
| **Inyección de dependencias** | creacional (IoC) | los constructores + el ensamblador (`crear_servicio_programa()`) |
...
# ServicioPrograma lo usa SIN saber si detrás hay PostgreSQL o un
...
```python
class ServicioPrograma:
    def __init__(self, repositorio: IRepositorioPrograma):  # ← llega armado
        self._repositorio = repositorio

# El ÚNICO lugar que conoce la clase concreta es el ensamblador:
def crear_servicio_programa() -> IServicioPrograma:
    return ServicioPrograma(RepositorioProgramaPostgreSQL(cadena))
```

### DTO por verbo — el body aterriza en un modelo que solo valida forma

```python
class Programa(BaseModel):                       # POST: todo obligatorio
    codigo: str = Field(min_length=1, max_length=20)
    stock: int = Field(ge=0)
    # Un body inválido muere en 422 ANTES de tocar servicio o BD.

class ProgramaActualizar(BaseModel):             # PATCH: todo opcional
    ...
```
...
def crear_repositorio_programa(motor: str) -> IRepositorioPrograma:
    if motor == "mariadb":
        return RepositorioProgramaMariaDB(cadena_mariadb)
    return RepositorioProgramaPostgreSQL(cadena_postgres)
...
(`RepositorioProgramaPostgreSQL`, el `RepositorioFalso` de la prueba — y
