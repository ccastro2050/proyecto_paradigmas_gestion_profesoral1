# Plan — construir la v1 del módulo Gestión Profesoral con la plantilla del curso

> **Qué es este documento.** El plan de trabajo para producir, en este
> repositorio, el **ejemplo de referencia** del módulo Gestión Profesoral: su
...
```mermaid
flowchart LR
    N["Navegador o Postman"] -->|"HTTP puerto 8029"| A["API gestion profesoral<br/>Python FastAPI + SQLAlchemy"]
    A -->|"SQL parametrizado"| B[("PostgreSQL puerto 15441<br/>base gestion_local")]
```
...
| Insumo | Qué aporta |
|---|---|
| `ProyectosDeAula/docs/modulo_gestion_profesoral.md` | El **QUÉ**: las 19 tablas, la ruta de 4 versiones, qué entra en cada una |
| `ProyectosDeAula/db_scripts/postgresql/gestion_profesoral.pg.sql` | El **DDL**: 223 líneas, 19 `CREATE TABLE`, 19 claves foráneas |
...
| 3 | Ninguna de las 16 tablas del módulo tiene columna `activo` (solo `rol` y `usuario`) | La metodología y la rúbrica exigen **borrado lógico** |
...
| 6 | **El Excel tiene 191 programas, pero solo cuatro de las once columnas de la tabla** —y seis de las que faltan no admiten nulos: `nivel`, `fecha_creacion`, `numero_cohortes`, `cant_graduados`, `fecha_actualizacion` y `ciudad` | **No se puede sembrar sin inventar datos.** La tabla de la v1 arranca vacía |
...
| 8 | **`programa.facultad` es un `INT` que apunta a una tabla que no existe en este módulo** | No hay integridad referencial que imponer: es un número más |
...
### 3.1 La v1 son cinco tablas; el ejemplo construye `programa`
...
|---|---|---|
| **`programa`** | **11** | 0 (el Excel trae 191, pero incompletos — hallazgo 6) |
| `area_conocimiento` | 4 | 218 |
| `red` | 4 | 0 |
| `linea_investigacion` | 3 | 0 |
| `termino_clave` | 2 | 0 |

Gana **`programa`** con once campos contra cuatro. Y es, además, la tabla
...
1. **Tiene un campo opcional entre diez obligatorios** (`fecha_cierre`: un
   programa abierto no la tiene). Eso hace que el contraste `PUT`/`PATCH`
...
|---|---|---|
| API gestión profesoral | **8029** | v1 |
| PostgreSQL | **15441** | v1 |
| **Front Blazor** | **8027** | la pantalla de esta misma versión |
...
```yaml
  api-gestion:
    environment:
      ConnectionStrings__PostgreSQL: "Server=postgres,1433;Database=gestion_local;User Id=sa;Password=Aplicacionweb123!;TrustServerCertificate=True;"
```
...
flowchart LR
    ...
    CMP --> API["api-gestion<br/>ConnectionStrings__PostgreSQL<br/>sobreescribe las variables del compose"]
...
```yaml
    environment:
      ...
      ConnectionStrings__PostgreSQL: "Server=postgres,1433;Database=gestion_local;User Id=sa;Password=${MSSQL_SA_PASSWORD};TrustServerCertificate=True;"
```
...
```
proyecto_paradigmas_gestion_profesoral1/
...
│           └── v1_programa/   ← 2_spec … 8_tasks · 9_checklist · GUIA_IA1
...
|---|---|---|
| `api_gestion\Controllers` | Los endpoints | **Capa 1 — HTTP**: traduce a códigos de estado y JSON |
| `api_gestion\Peticiones` | Una clase por verbo (crear, reemplazo, actualizar) | La **frontera de entrada**: lo que valida el cuerpo y produce los 422 |
| `api_gestion\Modelos` | La entidad `Programa` | Lo que viaja entre capas |
| `api_gestion\Servicios` | La interfaz y las reglas de negocio | **Capa 2 — negocio**: no conoce HTTP ni el motor |
| `api_gestion\Repositorios` | La interfaz y el SQL con SQLAlchemy | **Capa 3 — datos**: no conoce HTTP |
| `api_gestion\Excepciones` | `NoEncontradoExcepcion` | Cómo el negocio avisa un 404 sin hablar de HTTP |
| `api_gestion\pruebas` | La prueba de capas | Corre el servicio con un repositorio FALSO, **sin base de datos** |
...
```powershell
mkdir db, postman,
      api_gestion\Controllers, api_gestion\Modelos,
      api_gestion\Peticiones, api_gestion\Servicios,
      api_gestion\Repositorios, api_gestion\Excepciones,
      api_gestion\pruebas,
      docs\spec_kit\versiones\v1_programa
```
...
New-Item -ItemType File `
  api_gestion\ApiGestion/requirements.txt, api_gestion\Dockerfile,
  api_gestion\main.py, api_gestion\las variables del compose,
  api_gestion\Modelos\Programa.py,
  api_gestion\Peticiones\ProgramaCrear.py,
  api_gestion\Peticiones\ProgramaReemplazo.py,
  api_gestion\Peticiones\ProgramaActualizar.py,
  api_gestion\Repositorios\IRepositorioPrograma.py,
  api_gestion\Repositorios\RepositorioProgramaPostgreSQL.py,
  api_gestion\Servicios\IServicioPrograma.py,
  api_gestion\Servicios\ServicioPrograma.py,
  api_gestion\Controllers\ProgramaController.py,
  api_gestion\Excepciones\NoEncontradoExcepcion.py,
  api_gestion\pruebas\PruebaCapas/requirements.txt,
  api_gestion\pruebas\Programa.py

# el spec kit (pasos 3 y 4)
New-Item -ItemType File `
  docs\spec_kit\1_constitution.md,
  docs\spec_kit\versiones\0_mapa_versiones.md,
  docs\spec_kit\versiones\v1_programa\2_spec.md,
  docs\spec_kit\versiones\v1_programa\3_plan.md,
  docs\spec_kit\versiones\v1_programa\4_research.md,
  docs\spec_kit\versiones\v1_programa\5_data_model.md,
  docs\spec_kit\versiones\v1_programa\6_contracts.md,
  docs\spec_kit\versiones\v1_programa\7_quickstart.md,
  docs\spec_kit\versiones\v1_programa\8_tasks.md,
  docs\spec_kit\versiones\v1_programa\9_checklist.md,
  docs\spec_kit\versiones\v1_programa\GUIA_IA1.md
...
aula: `modulo_gestion_profesoral.md` dice el QUÉ y `0_METODOLOGIA.md` dice CÓMO
se trabaja.
...
| `docs/spec_kit/versiones/0_mapa_versiones.md` | La ruta v1→v4 que ya define `modulo_gestion_profesoral.md`, con el estado de cada versión |
| `v1_programa/2_spec.md` | El QUÉ de la v1: propósito, alcance con su NO incluye, requisitos, criterios medibles, **Clarificaciones** y definición de TERMINADA |
...
- **Clarificaciones** en `2_spec.md`: los hallazgos **1, 2 y 4** de la
  sección 2 —los que tocan a `programa`— cada uno con su
...
docker compose exec postgres bash -c '/opt/mssql-tools18/bin/sqlcmd `
  ...
  -Q "SELECT ''programa'' t, COUNT(*) n FROM programa
...
[`7_quickstart.md`](docs/spec_kit/versiones/v1_programa/7_quickstart.md)
...
### Paso 5 — La API
Python / FastAPI + SQLAlchemy, capas con interfaces, para `programa`: el modelo, las tres peticiones (crear / reemplazo / actualizar), la interfaz y el repositorio, la interfaz y el servicio, y el controlador con los siete endpoints. Más `main.py` con el ensamblador, `Dockerfile` y el proyecto `pruebas/` con un repositorio falso en memoria. **Unos 12 archivos**, no 45: esa es la ganancia de haber escogido una sola tabla.
```mermaid
flowchart TB
    C["Controlador<br/>HTTP: codigos y JSON"] --> IS["IServicioPrograma<br/>interfaz"]
    S["ServicioPrograma<br/>reglas de negocio"] -.->|implementa| IS
    S --> IR["IRepositorioPrograma<br/>interfaz"]
    R["RepositorioProgramaPostgreSQL<br/>SQL con SQLAlchemy"] -.->|implementa| IR
    R --> BD[("tabla programa")]
...
```powershell
python -m compileall api_gestion
uvicorn --project api_gestion\pruebas
```
...
curl http://localhost:8029/                                  # diagnostico: version v1
curl http://localhost:8029/api/programa             # total: 218
curl "http://localhost:8029/api/programa?limite=3"  # exactamente 3
curl http://localhost:8029/api/programa/1A01        # una fila
curl -i http://localhost:8029/api/programa/9Z99     # 404 con mensaje claro
```
...
```powershell
git add -A
git commit -m "v1: CRUD de programa con capas e interfaces"
git tag v1
...
`v1_programa/` (`2_spec` a `8_tasks`). El `9_checklist.md` **no**
...
4. La base de datos YA VIENE DADA en db/init.sql + db/iid.sh —
   ...
   escribirlo es tu primera tarea (Fase 0). La tabla programa ya
...
8. Trabajo en Windows con VS Code (terminal integrada de PowerShell) y
   ...
   puerto 8029 y PostgreSQL el 15441.
...
(1_constitution.md en la raíz; los demás en versiones/v1_programa/):
...
El código va en api_gestion/ según la estructura de 3_plan.md.
...
5. Todo en español, Python sobre FastAPI con SQLAlchemy y el SQL a
   la vista. API en el puerto 8029, PostgreSQL en el 15441.
...
> aquí llevan el prefijo `paradigmas-gestion-`, y la regla quedó en las convenciones
> de la constitución.
